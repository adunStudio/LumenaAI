...
class YouTubeGenerateTimelineSummary(YoutubeUseCase):
    ...
    def execute(self, youtube_url: str, **kwargs) -> ExecuteResult:
        ...
        # 7. 핵심 용어 생성
        youtube_url = content.url.url
        key_points = [
            YoutubeKeyPoint(term=kp.term, description=kp.description)
            for kp in response.key_points
        ]

        key_point_collection:YoutubeKeyPointCollection = YoutubeKeyPointCollection(youtube_url, key_points)
